...
create table if not exists profile_permissions (
  ...
  admin_badside_ids uuid[] not null default '{}',
  member_badside_ids uuid[] not null default '{}',
...
-- ------------------------------------------------------------
-- 3. role_mappings — pemetaan Discord Role ID -> badside/workshop
-- Contoh isi (lihat README untuk cara isi lewat Table Editor):
--   discord_role_id='111...', type='member_badside', badside_id=<id Gravencio>
--   discord_role_id='222...', type='admin_badside',  badside_id=<id Gravencio>
...
create table if not exists role_mappings (
  ...
  label text not null,
  type text not null check (type in ('member_badside','admin_badside','member_workshop','admin_workshop','super_admin')),
...
create or replace function is_admin_badside(bid uuid) returns boolean as $$
  select is_super_admin() or coalesce((select bid = any(admin_badside_ids) from profile_permissions where id = auth.uid()), false);
$$ language sql security definer stable;

create or replace function is_member_badside(bid uuid) returns boolean as $$
  select is_admin_badside(bid) or coalesce((select bid = any(member_badside_ids) from profile_permissions where id = auth.uid()), false);
$$ language sql security definer stable;
...
create or replace function is_any_badside_admin() returns boolean as $$
  select is_super_admin() or coalesce((select array_length(admin_badside_ids,1) > 0 from profile_permissions where id = auth.uid()), false);
$$ language sql security definer stable;
...
-- INSERT/UPDATE/DELETE dibatasi sesuai peran.
-- ------------------------------------------------------------

-- BADSIDE (entitas induk): hanya super admin yang bisa membuat/menghapus,
-- admin badside terkait boleh mengubah data badside-nya sendiri.
...
-- ubah/hapus hanya admin badside.
...
-- lamaran_mekanik: siapapun yang login boleh melamar (insert), admin workshop yang proses
