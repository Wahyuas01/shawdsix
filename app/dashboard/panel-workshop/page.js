      ...
      <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
        ...
        <table className="w-full text-sm">
          <tbody className="divide-y divide-slate-100">
            {(dutySaya || []).map((d) => (
              <tr key={d.id}>
                <td className="px-5 py-3 text-slate-500 whitespace-nowrap align-top">{d.tanggal}</td>
                <td className="px-5 py-3">
                  {d.jam_mulai || '—'} - {d.jam_selesai || '—'}
                  {d.catatan ? ` — ${d.catatan}` : ''}
                  {(d.foto_on_duty_url || d.foto_off_duty_url) && (
                    <div className="flex gap-2 mt-2">
                      {d.foto_on_duty_url && (
                        <a href={d.foto_on_duty_url} target="_blank" rel="noreferrer">
                          <img src={d.foto_on_duty_url} alt="On Duty" className="w-14 h-14 rounded-lg object-cover border border-slate-200" />
                        </a>
                      )}
                      {d.foto_off_duty_url && (
                        <a href={d.foto_off_duty_url} target="_blank" rel="noreferrer">
                          <img src={d.foto_off_duty_url} alt="Off Duty" className="w-14 h-14 rounded-lg object-cover border border-slate-200" />
                        </a>
                      )}
                    </div>
                  )}
