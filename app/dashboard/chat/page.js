...
export default function ChatPage() {
  ...
  async function send(e) {
    ...
    await supabase.from('chat_messages').insert({ sender_name: name, pesan: text, channel: 'general' });
    setText('');
  }
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl flex flex-col h-[calc(100vh-140px)]">
      ...
      <form onSubmit={send} className="border-t border-slate-200 p-3 flex gap-2">
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Nama kamu" className="border border-slate-200 rounded-lg px-3 py-2 text-sm w-32" required />
        <input value={text} onChange={(e) => setText(e.target.value)} placeholder="Tulis pesan..." className="border border-slate-200 rounded-lg px-3 py-2 text-sm flex-1" required />
        <button type="submit" className="bg-brandblue-600 hover:bg-brandblue-700 text-white text-sm font-semibold px-4 rounded-lg">Kirim</button>
      </form>
    </div>
  );
}
